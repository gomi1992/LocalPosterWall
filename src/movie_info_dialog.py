...
class MovieInfoDialog(QDialog):
    ...
    def _create_scroll_container(self, parent_layout):
        ...
        self.scroll_container.setMinimumSize(450, 200)
    ...
    def _create_info_labels(self, layout):
        ...
        director_text = self.movie_info.get('director', '未知导演')
        director = QLabel(f"导演：{director_text}")
        director.setWordWrap(True)
        director.setStyleSheet("""
                    QLabel {
                        font-size: 14px;
                        color: #666;
                        padding: 3px;
        ...
        logger.debug(f"导演标签创建: {director_text}")

        actor_text = self.movie_info.get('actor', '未知演员')
        actor = QLabel(f"导演：{actor_text}")
        actor.setWordWrap(True)
        actor.setStyleSheet("""
                            QLabel {
                                font-size: 14px;
                                color: #666;
        ...
        logger.debug(f"演员标签创建: {actor_text}")
